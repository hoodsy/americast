...
Filter per plan: state == CA, status == OP, technology == Solar
...
Gate 1 labels (CAISO fuel mix) measure the **balancing authority**, not
...
## Open decision (Gate 2 review)

Whether Gate 3 weather sampling should use CA-only plants (current
registry, per plan) or the full CISO fleet including AZ/NV (matches what
the labels measure; ~10% of capacity). Registry filter is one line either
